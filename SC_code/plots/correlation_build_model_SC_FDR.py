...
def read_MS_and_merge_with_FDR(subject, FDR_df, uniform= False, ratio = '', source_for_faults ='',  group_name=''):
    ...
    source_path_for_ms = os.path.join(root, 'SC', 'all_results',subject, str(ratio), source_for_faults, prefix)
...
def calculate_correlation(df, SC = 'DSC', FDR= 'FDR'):
    '''
    inputs
        df: dataframes include two columns
        SC: name of SC column in df
    ...
    df[SC] = pd.to_numeric(df[SC], errors='coerce').astype(float)
    
    pearson_corr = df[FDR].corr(df[SC], method='pearson')
    
    output[f'pearson_corr'] = round(pearson_corr,2)
    
    
    # compute Spearman correlation
    spearman_corr = df[FDR].corr(df[SC], method='spearman')
    output[f'spearman_corr'] = round(spearman_corr, 2)

    # compute Kendall correlation
    kendall_corr = df[FDR].corr(df[SC], method='kendall')
...
def fit_linear_regression_and_statistical_results(df, subject= '', group_name = '', type_of_sampling= False, exp = ''):
    ...
    data_df = pd.DataFrame({'SC':X.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'ci_lower':y_pred_lower_bound, 'ci_upper':y_pred_upper_bound})

    
    a = sns.lineplot(data=data_df, x="SC", y="predicted_FDR", linewidth = 0.9, color='red')
    b = sns.lineplot(data=data_df, x="SC", y="ci_lower", color='b', linewidth = 0.4)
    c = sns.lineplot(data=data_df, x="SC", y="ci_upper", color='b', linewidth = 0.4)
    ...
    # df = pd.DataFrame({'SC':X, 'FDR':y})
    # sns.scatterplot(data=df, x=x_MS, y="FDR",  hue="test_suite_size", style="test_suite_size").set(title=f'{subject}_{group_name}_{x_MS}')
    sns.scatterplot(data=df, x="SC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors, s=10).set(title=f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
...
def fit_transformed_regression_and_statistical_results(df, subject= '', group_name = '', type_of_sampling= False, exp = '', type_of_transformation = 1):
    ...
    data_df = pd.DataFrame({'SC':X_t.reshape(-1),
               'FDR':y_t.reshape(-1)})
    
    pi_model = smf.ols('FDR ~ SC', df)
    ...
    # data_df = pd.DataFrame({'SC':X_t.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'ci_lower':y_pred_lower_bound.reshape(-1), 'ci_upper':y_pred_upper_bound.reshape(-1)})
    data_df = pd.DataFrame({'SC':X_t.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'ci_lower':y_pred_lower_bound, 'ci_upper':y_pred_upper_bound})
    ...
    # df = pd.DataFrame({'SC':X, 'FDR':y})
    # sns.scatterplot(data=df, x=x_MS, y="FDR",  hue="test_suite_size", style="test_suite_size").set(title=f'{subject}_{group_name}_{x_MS}')
    # sns.scatterplot(data=df, x="SC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors, s=20).set(title=f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
    sns.scatterplot(data=df, x="SC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors)
...
def fit_polynomial_regression_and_statistical_results(df, subject= '', group_name = '', type_of_sampling= False, exp = ''):
    ...
    # df = pd.DataFrame({'SC':X, 'FDR':y})
    # sns.scatterplot(data=df, x=x_MS, y="FDR",  hue="test_suite_size", style="test_suite_size").set(title=f'{subject}_{group_name}_{x_MS}')
    sns.scatterplot(data=df, x="SC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors).set(title=f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
...
def fit_RT(df, subject= '', group_name = '', type_of_sampling= False, exp = ''):
    ...
    # _ = tree.plot_tree(model, feature_names=['SC'], filled=True)
...
def fit_RegTree(df, subject= '', group_name = '', type_of_sampling= False, exp = ''):
    ...
    # df = pd.DataFrame({'SC':X, 'FDR':y})
    # sns.scatterplot(data=df, x=x_MS, y="FDR",  hue="test_suite_size", style="test_suite_size").set(title=f'{subject}_{group_name}_{x_MS}')
    # sns.scatterplot(data=df, x="SC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors, s=20,  alpha=.9).set(title=f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
    sns.scatterplot(data=df, x="SC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors,  alpha=.9)
    
    
    # X = 1/X
    # plt.scatter(X, y)
    # draw upper_confidence
    data_df = pd.DataFrame({'SC':X.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'ci_lower':confidence_intervals[0].reshape(-1), 'ci_upper':confidence_intervals[1].reshape(-1)})
    # data_df_pi = pd.DataFrame({'SC':X.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'pi_lower':prediction_intervals[0].reshape(-1), 'pi_upper':prediction_intervals[1].reshape(-1)})
    data_df_pi = pd.DataFrame({'SC':X.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'pi_lower':y_pred_lower_bound.reshape(-1), 'pi_upper':y_pred_upper_bound.reshape(-1)})
    
    
    # fig, ax = plt.subplots()
    # # ax.plot(data_df['SC'], data_df['predicted_FDR'])
    
    
    
    # sns.lineplot(data=data_df, x="SC", y="predicted_FDR")
    # plt.fill_between(data_df['SC'], data_df['ci_lower'], data_df['ci_upper'], color='b', alpha=.15)
    ...
    a = sns.lineplot(data=data_df, x="SC", y="predicted_FDR", linewidth = 0.9, color='red')
    # b = sns.lineplot(data=data_df, x="SC", y="ci_lower", color='b', linewidth = 0.3)
    # c = sns.lineplot(data=data_df, x="SC", y="ci_upper", color='b', linewidth = 0.3)

    # line = c.get_lines()
    # plt.fill_between(line[0].get_xdata(), line[1].get_ydata(), line[2].get_ydata(), color='b', alpha=.25)
    
    d = sns.lineplot(data=data_df_pi, x="SC", y="pi_lower", color='b', linewidth = 0.5)
    e = sns.lineplot(data=data_df_pi, x="SC", y="pi_upper", color='b', linewidth = 0.5)
    ...
    # sns.lineplot(data=pd.DataFrame({'SC':X.reshape(-1), 'predicted_FDR':confidence_intervals[0].reshape(-1)}), x="SC", y="predicted_FDR")
    # sns.lineplot(data=data_df, x="SC", y="predicted_FDR")
    # sns.lineplot(data=pd.DataFrame({'SC':X.reshape(-1), 'predicted_FDR':confidence_intervals[1].reshape(-1)}), x="SC", y="predicted_FDR")
...
def run_normal(subjects, shifted_test_input):
    for subject in subjects:
        ...
        column_name_mapping = {0: 'DSC_random',
                1: 'LSC_random',
                # 2: 'E3_random'
                }
