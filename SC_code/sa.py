...
def get_ats(
    ...
    if is_classification:
        ...
        #### pred = model.predict_classes(dataset, batch_size=batch_size, verbose=1) ## deprecated so we chnaged it to bellow
        pred = np.argmax(model.predict(dataset, batch_size=batch_size), axis=-1)
...
def _get_train_target_ats(model, x_train, x_target, target_name, layer_names, args):
    ...
    print('elapsed time to calculate ATS for train is:', e1-s1 )
    
    saved_target_path = _get_saved_path(
        args.save_path, args.d, target_name, layer_names
    )
    if os.path.exists(saved_target_path[0]):
        print(infog("Found saved {} (target) ATs, skip serving").format(target_name))
        ...
        print(infog(target_name + "(target) ATs is saved at " + saved_target_path[0]))

    
    print(f'elapsed time to calculate ATS for {target_name} as a target is:', datetime.now() - e1 )
...
def fetch_dsa(model, x_train, x_target, target_name, layer_names, args):
    ...
    for i, at in enumerate(tqdm(target_ats)):
        ...
        ats_of_same_classes = train_ats[class_matrix[label]]
        
        if target_name == 'train':
            # remove the at point from training Ats because we suppose at is new data 
            indices = np.where((ats_of_same_classes == at).all(axis=1))[0]
            ats_of_same_classes = np.delete(ats_of_same_classes, indices[0], axis=0)
            
        a_dist, a_dot = find_closest_at(at, ats_of_same_classes)
    ...
    print('elapsed time to fetch DSA =', e1 - s1)
    
    return dsa


def _get_kdes(train_ats, train_pred, class_matrix, args, at_target_index = None):
    ...
    removed_cols = [] ## list of removed neurons
    if args.is_classification:
        for label in range(args.num_classes):
            # train_ats : 50000, 512
            # class_matrix[label] = [34,56,789, ... ] so the len is 512
            col_vectors = np.transpose(train_ats[class_matrix[label]]) # (512, 473))
        ...
        for label in range(args.num_classes):
            # if at_target_index in class_matrix[label]:
            #     class_matrix[label].remove(at_target_index)   
...
def _get_lsa(kde, at, removed_cols):
    ...
    # return np.asscalar(-kde.logpdf(np.transpose(refined_at))) ## deprecated
    return (-kde.logpdf(np.transpose(refined_at))).item()
...
def fetch_lsa(model, x_train, x_target, target_name, layer_names, args):
    ...
    if args.is_classification:
        for i, label in enumerate(train_pred):
            if label not in class_matrix:
                class_matrix[label] = []
            class_matrix[label].append(i) ## class_matrix len is 50000 , point that which samples (indexes) are for label
    # print('===========================')
    # print(np.array(train_ats).shape)
    # print(np.array(train_pred).shape)
    # print(len(class_matrix))
    # print('===========================')
    
    s = datetime.now()
    kdes, removed_cols = _get_kdes(train_ats, train_pred, class_matrix, args)
    e = datetime.now()
    print('timeeeeeeeeeeeeee:', e-s)
    ...
    if args.is_classification:
        for i, at in enumerate(tqdm(target_ats)):
            ...
            if target_name == 'train':
                # remove the at point from training Ats because we suppose at is new data 
                indices = np.where((train_ats == at).all(axis=1))[0]
                train_ats_without_target_point = np.delete(train_ats, indices[0], axis=0)
                train_pred_without_target_point = np.delete(train_pred, indices[0], axis=0)
                
                class_matrix = {}
                if args.is_classification:
                    for i, label in enumerate(train_pred_without_target_point):
                        if label not in class_matrix:
                            class_matrix[label] = []
                        class_matrix[label].append(i)
                        
                kdes, removed_cols = _get_kdes(train_ats_without_target_point, train_pred_without_target_point, class_matrix, args)
                # kdes, removed_cols = _get_kdes(train_ats, train_pred, class_matrix, args, at_target_index = indices[0])
    ...
    print('elapsed time to fetch LSA =', e1 - s1)
