...
parser.add_argument('--dataset', type=str,
                            default="mnist", choices=["mnist", "cifar10", "fmnist", "cifar100", "svhn"],
                            help='dataset')
...
if args.dataset == "mnist":
    ...
    testset = dset.CIFAR100(root="./data", train = False, download=True)
    dataset_cifar = dset.CIFAR100(root="./data", download=True, train = True, transform=transforms.Compose([transforms.Resize(32), transforms.ToTensor()]))
    test_loader = torch.utils.data.DataLoader(dataset_cifar, batch_size=args.batchsize, shuffle=False)
    
elif args.dataset == "svhn":
    # testset = dset.SVHN(root="./data", train = False, download=True)
    dataset_cifar = dset.SVHN(root="./data", download=True, split = 'test', transform=transforms.Compose([transforms.Resize(32), transforms.ToTensor()]))
    test_loader = torch.utils.data.DataLoader(dataset_cifar, batch_size=args.batchsize, shuffle=False)
