...
class CIFAR100(datasets.CIFAR100):
    """CIFAR-100 wrapper. Docs: `datasets.CIFAR100.`"""
    img_size = (3, 32, 32)  # CIFAR-100 images are RGB, hence 3 channels
    background_color = COLOUR_WHITE  # CIFAR-100 has colorful images, no single background color

    def __init__(self, root=os.path.join('..', 'data', 'cifar100'), train=True, download=True, **kwargs):
        super().__init__(root,
                         train=train,
                         download=download,
                         transform=transforms.Compose([
                             transforms.Resize(32),  # CIFAR-100 images are already 32x32, so this is redundant
                             transforms.ToTensor()
                         ]))
...
class SVHN(datasets.SVHN):
    ...
    img_size = (3, 32, 32)  # SVHN images are RGB, hence 3 channels
    background_color = COLOUR_WHITE  # SVHN images have various backgrounds

    def __init__(self, root=os.path.join('..', 'data', 'svhn'), split='train', download=True, **kwargs):
        super().__init__(root,
                         split=split,
                         download=download,
                         transform=transforms.Compose([
                             transforms.Resize(32),  # SVHN images are already 32x32, so this is redundant
                             transforms.ToTensor()
                         ]))
...
class ImageNet(Dataset):
    ...
    def __init__(self, root=os.path.join('datasets', 'imagenet32x32'), split='train', download=False, **kwargs):
        self.data = []
        self.labels = []
        root = "datasets/imagenet32x32"
        # Load all batch files
        if split== 'train':
            for i in range(1, 11):  # Assuming there are 10 training batch files
                batch = unpickle(os.path.join(root, 'Imagenet32_train', f'train_data_batch_{i}'))
                self.data.append(batch['data'])
                self.labels.extend(batch['labels'])
            self.data = np.vstack(self.data).reshape(-1, 3, 32, 32).transpose((0, 2, 3, 1))  # reshape and permute axes to HWC
        else:
            val_data = unpickle(os.path.join(root, 'Imagenet32_val','val_data'))
            self.data = val_data['data'].reshape(-1, 3, 32, 32).transpose((0, 2, 3, 1))
    ...
    # root_dir = 'datasets/imagenet32x32'
    # train_dataset = self(root_dir, train=False)
