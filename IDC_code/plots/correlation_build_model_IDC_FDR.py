...
def read_FDRs(subject, uniform= False, source_for_faults ='', group_name=''):
    ...
    source_path_for_fdr = os.path.join(root,'..','MS_code','FDR_calculation', 'FDR_output', source_for_faults, subject, prefix)
...
def read_MS_and_merge_with_FDR(subject, FDR_df, uniform= False, ratio = '', source_for_faults ='',  group_name=''):
    ...
    source_path_for_ms = os.path.join(root, 'IDC', 'all_results',subject, str(ratio), source_for_faults, prefix)
    ...
    for bs_path in result_data_list:
        ...
        df_path = os.path.join(source_path_for_ms, bs_path, f'{prefix}_result_SC.csv')
...
def calculate_correlation(df, IDC = 'IDC', FDR= 'FDR'):
    '''
    inputs
        df: dataframes include two columns
        IDC: name of IDC column in df
    ...
    df[IDC] = pd.to_numeric(df[IDC], errors='coerce').astype(float)
    
    pearson_corr = df[FDR].corr(df[IDC], method='pearson')
    
    output[f'pearson_corr'] = round(pearson_corr,2)
    
    
    # compute Spearman correlation
    spearman_corr = df[FDR].corr(df[IDC], method='spearman')
    output[f'spearman_corr'] = round(spearman_corr, 2)

    # compute Kendall correlation
    kendall_corr = df[FDR].corr(df[IDC], method='kendall')
...
def fit_linear_regression_and_statistical_results(df, subject= '', group_name = '', type_of_sampling= False, exp = ''):
    ...
    data_df = pd.DataFrame({'IDC':X.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'ci_lower':y_pred_lower_bound, 'ci_upper':y_pred_upper_bound})

    
    a = sns.lineplot(data=data_df, x="IDC", y="predicted_FDR", linewidth = 0.9, color='red')
    b = sns.lineplot(data=data_df, x="IDC", y="ci_lower", color='b', linewidth = 0.4)
    c = sns.lineplot(data=data_df, x="IDC", y="ci_upper", color='b', linewidth = 0.4)
    ...
    # df = pd.DataFrame({'IDC':X, 'FDR':y})
    # sns.scatterplot(data=df, x=x_MS, y="FDR",  hue="test_suite_size", style="test_suite_size").set(title=f'{subject}_{group_name}_{x_MS}')
    sns.scatterplot(data=df, x="IDC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors, s=10).set(title=f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
    ...
    save_fig_path = os.path.join(save_fig_path, f'{title}_{exp}_linear_regression.png')
...
def fit_transformed_regression_and_statistical_results(df, subject= '', group_name = '', type_of_sampling= False, exp = '', type_of_transformation = 1):
    ...
    data_df = pd.DataFrame({'IDC':X_t.reshape(-1),
               'FDR':y_t.reshape(-1)})
    
    pi_model = smf.ols('FDR ~ IDC', df)
    ...
    # data_df = pd.DataFrame({'IDC':X_t.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'ci_lower':y_pred_lower_bound.reshape(-1), 'ci_upper':y_pred_upper_bound.reshape(-1)})
    data_df = pd.DataFrame({'IDC':X_t.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'ci_lower':y_pred_lower_bound, 'ci_upper':y_pred_upper_bound})
    ...
    # df = pd.DataFrame({'IDC':X, 'FDR':y})
    # sns.scatterplot(data=df, x=x_MS, y="FDR",  hue="test_suite_size", style="test_suite_size").set(title=f'{subject}_{group_name}_{x_MS}')
    # sns.scatterplot(data=df, x="IDC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors, s=20).set(title=f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
    sns.scatterplot(data=df, x="IDC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors)
...
def fit_polynomial_regression_and_statistical_results(df, subject= '', group_name = '', type_of_sampling= False, exp = ''):
    ...
    # df = pd.DataFrame({'IDC':X, 'FDR':y})
    # sns.scatterplot(data=df, x=x_MS, y="FDR",  hue="test_suite_size", style="test_suite_size").set(title=f'{subject}_{group_name}_{x_MS}')
    sns.scatterplot(data=df, x="IDC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors).set(title=f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
...
def fit_RT(df, subject= '', group_name = '', type_of_sampling= False, exp = ''):
    ...
    # _ = tree.plot_tree(model, feature_names=['IDC'], filled=True)
...
def fit_RegTree(df, subject= '', group_name = '', type_of_sampling= False, exp = ''):
    ...
    # df = pd.DataFrame({'IDC':X, 'FDR':y})
    # sns.scatterplot(data=df, x=x_MS, y="FDR",  hue="test_suite_size", style="test_suite_size").set(title=f'{subject}_{group_name}_{x_MS}')
    # sns.scatterplot(data=df, x="IDC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors, s=20,  alpha=.9).set(title=f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
    sns.scatterplot(data=df, x="IDC", y="FDR",  hue="test_suite_size", style="test_suite_size", palette = colors,  alpha=.9)
    
    
    # X = 1/X
    # plt.scatter(X, y)
    # draw upper_confidence
    data_df = pd.DataFrame({'IDC':X.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'ci_lower':confidence_intervals[0].reshape(-1), 'ci_upper':confidence_intervals[1].reshape(-1)})
    # data_df_pi = pd.DataFrame({'IDC':X.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'pi_lower':prediction_intervals[0].reshape(-1), 'pi_upper':prediction_intervals[1].reshape(-1)})
    data_df_pi = pd.DataFrame({'IDC':X.reshape(-1), 'predicted_FDR':y_pred.reshape(-1), 'pi_lower':y_pred_lower_bound.reshape(-1), 'pi_upper':y_pred_upper_bound.reshape(-1)})
    
    
    # fig, ax = plt.subplots()
    # # ax.plot(data_df['IDC'], data_df['predicted_FDR'])
    
    
    
    # sns.lineplot(data=data_df, x="IDC", y="predicted_FDR")
    # plt.fill_between(data_df['IDC'], data_df['ci_lower'], data_df['ci_upper'], color='b', alpha=.15)
    ...
    a = sns.lineplot(data=data_df, x="IDC", y="predicted_FDR", linewidth = 0.9, color='red')
    # b = sns.lineplot(data=data_df, x="IDC", y="ci_lower", color='b', linewidth = 0.3)
    # c = sns.lineplot(data=data_df, x="IDC", y="ci_upper", color='b', linewidth = 0.3)

    # line = c.get_lines()
    # plt.fill_between(line[0].get_xdata(), line[1].get_ydata(), line[2].get_ydata(), color='b', alpha=.25)
    
    d = sns.lineplot(data=data_df_pi, x="IDC", y="pi_lower", color='b', linewidth = 0.5)
    e = sns.lineplot(data=data_df_pi, x="IDC", y="pi_upper", color='b', linewidth = 0.5)
    ...
    # sns.lineplot(data=pd.DataFrame({'IDC':X.reshape(-1), 'predicted_FDR':confidence_intervals[0].reshape(-1)}), x="IDC", y="predicted_FDR")
    # sns.lineplot(data=data_df, x="IDC", y="predicted_FDR")
    # sns.lineplot(data=pd.DataFrame({'IDC':X.reshape(-1), 'predicted_FDR':confidence_intervals[1].reshape(-1)}), x="IDC", y="predicted_FDR")
...
def evaulate_regression_model(model, df, subject = '', type_of_sampling = False, group_name='', exp ='', type_of_regression = 'linear', type_of_transformation = 1):
    ...
    for has_intercept in [True, False]:
        ...
        plt.ylabel('Actual FDR', fontsize=9)
        plt.xlabel('Predicted FDR', fontsize=9)
        # plt.title(f'[{subject}]_[{group_name}]_[{type_of_sampling}]')
...
def run(subject, uniform, group_name=None, shifted_test_input = ''):
    ...
    # for exp in ['E1']:
        ...
        make_plot(df, f'{exp}', subject, uniform, group_name)

        corr = calculate_correlation(df, f'{exp}', 'FDR')
        ...
        exp_corr[exp+'__'+sampling_type] = corr
        model_eval_result[exp+'__'+sampling_type] = corr_eval
