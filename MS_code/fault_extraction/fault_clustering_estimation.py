...
logger = MyLogger('fault_clustering_estimation.py') 
...
def run_clustering(x_mis, tt, tst, mis_index, mis_tindex, key):
    print('start run clustering')
    print('load x_mis from trian successfully!')
    ...
    for metric in ['euclidean']:
        ...
        for i,j in zip([1024,250,200,150,100],[512,200,150,100,50]):  # double reduction ## you can increase the number of reduction list
            for k,o in zip([3,4,5,6],[3,4,5,6]): # k-neighbors
                for n_n in [0.001, 0.01, 0.1]:
                    ...
                    try:
                        ...
                        # first_umap_reducer = umap.UMAP(n_components=)
                        # first_umap_reducer = umap.UMAP()
                        u1 = first_umap_reducer.fit_transform(x_mis)
                        
                        print('===============1===============')
                        
                        # dimension reduction u1 (i D) -> u (j D)
                        second_umap_reducer = umap.UMAP(min_dist=n_n, n_components=j, n_neighbors=o, metric=metric)
                        u = second_umap_reducer.fit_transform(u1)
                        
                        print('==============2================')
                    except:
                        print('--- exception in umap dimention reduction ---')
                        continue
                    # u = np.c_[u, tt_scale]
                    # u = np.c_[u, tst_scale]
                    ...
                    for min_cluster_size in [3,4,5]: # minimum cluster size 
                        ...
                        print("the number of findings cluster ", len(list(Clustering_Label)))
                        ...
                        try:
                            clustering_score = sklearn.metrics.silhouette_score(u, labels)
                        except:
                            continue
                        ...
                        # if True:
                        if (list(labels).count(-1) <= len(x_mis) * 0.10): ## the number of noise should be less than 10% of mispredicteds  
...
if __name__ == '__main__':
    ...
    if source_data_for_clustering == 'training': # need clustering
        ...
        if do_clustering:
            ...
            clusters, all_clusters_info = run_clustering(x_mis_train, tt, tst, mis_index, mis_tindex, key)
